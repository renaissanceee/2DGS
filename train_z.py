...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, source_z):
    first_iter = 0
    HR = dataset.resolution
    # dataset.resolution = 1
    dataset.resolution = 8
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians, source_z=source_z)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        #--------------------------
        if viewpoint_cam.uid<100:# 0-99:far, 100-199:near
            # -r 800,1600,3200: 800*800
            viewpoint_cam.image_height, viewpoint_cam.image_width = HR, HR
            patch_size = viewpoint_cam.image_height // 800

            # -r 1,2,4: 100*100
            # viewpoint_cam.image_height, viewpoint_cam.image_width = 800//HR, 800//HR
            # patch_size = 8 // HR
            render_pkg = render(viewpoint_cam, gaussians, pipe, background)
            image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
            image = F.avg_pool2d(image.unsqueeze(0), kernel_size=patch_size, stride=patch_size)
        else:
            render_pkg = render(viewpoint_cam, gaussians, pipe, background)
            image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]
        # --------------------------
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations,
             args.checkpoint_iterations, args.start_checkpoint, args.source_z)
